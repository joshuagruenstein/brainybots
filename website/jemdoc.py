...
def procfile(f):
  ...
  # hacky subdirectory count
  subdirs = len(list(filter(
    lambda x: x not in ['.',''],
    os.path.split(os.path.dirname(f.inname))
  )))
  ...
  if menu:
    ...
    menu = (menu[0], menu[1], os.path.join(os.path.dirname(f.inname),menu[2])[2:], '../'*subdirs)
...
#    print(infile.read())
    ...
    print(infile, outfile, conf, inname)
